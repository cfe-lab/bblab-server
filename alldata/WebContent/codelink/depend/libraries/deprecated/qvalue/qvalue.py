...
class QValue(object):
    ...
    def estimate_pi0(self, pi0):
        ...
        if pi0 is not None:
            ...
            pi0 = min(pi0, 1)
            print("case: len(self.lambdas) == 1<br>")
        else:
            print("case: pi0 is None<br>")
            # evaluate pi0 for different lambdas
            print("pv = {}<br>".format(pv))
            pi0 = [np.mean(pv>=this)/(1-this) for this in self.lambdas]
            print ("first pi0 = {}<br>".format(pi0))
            # in R
            ...
            if self.method == 'smoother':
                if (self.smooth_log_pi0):
                    pi0 = np.log(pi0)
                    print ("aftersmoothe1 pi0 = {}<br>".format(pi0))
                ...
                print ("afterinterpolate1 pi0 = {}<br>".format(pi0))
                pi0 = scipy.interpolate.splev(self.lambdas[-1], tck)
                print ("afterinterpolate2 pi0 = {}<br>".format(pi0))
                if (self.smooth_log_pi0):
                    pi0 = np.exp(pi0)
                print ("nextlast pi0 = {}<br>".format(pi0))
                pi0 = min(pi0, 1.)
            elif self.method == 'bootstrap':
                raise NotImplementedError
                """there used to be code in a comment here --gabe"""
                print("<br>This should NOT be called<br>")
            if pi0 > 1:
                if self.verbose:
                    print("got pi0 > 1 (%.3f) while estimating qvalues, setting it to 1" % pi0)

                pi0 = 1.0
        print ("last pi0 = {}<br>".format(pi0))
