...
sys.path.append( "{}/numpy-base-1.9.0-py27_0/lib/python3.7/site-packages/".format(PKGS_PATH) )  # Add the path to numpy (scipy dependency).
...
''' # I'm not using this anymore; validation will either reside in the main file or in the tests_util file.
def validate_data( protein_sequences ):
	"""
	This function makes sure the protein sequence data is proper.
	Will return a list of errors & warnings, or an empty list if none.
	"""
...
def get_output_matrix( protein_sequences, min_count):
	...
	def _run_seq_test( column_data ):
		...
		p_value = math_utils.round_to_sig_figs(stats.kruskal(w_decimal_list, nw_decimal_list)[1], 4)  # Use scipy to find the p-values.	
