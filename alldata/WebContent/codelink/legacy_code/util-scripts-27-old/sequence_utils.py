...
# See: ( https://github.com/emartin-cfe/emartin-personal/blob/master/PYTHON/miseqUtils.py ) for original file.  (More functions)
...
def consensus(column, alphabet='ACGT', resolve=False):
	...
	# Get a base with the highest frequenc
...
valid_protein_character_list = [ 'A', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y', '-', '_' ]  # This does not include B, J, O, U, or Z. 
...
def seq_start_test(in_sequence):
	...
	'''
	first_char = translate_nuc(in_sequence, 0)[0]
...
def seq_internal_test(in_sequence):
	...
	'''
	nuc_seq = translate_nuc(in_sequence, 0)  # Convert the dna seq to nuc. 
