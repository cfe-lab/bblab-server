...
def run(csv_data, analysis_id, email_address_string):
    ...
    try:
        ...
        plot_mod.create_isoforms_plot(input_stream, OUTPUT_SVG)
    except Exception as exc:
    ...
    subject_line = "Isoforms Plot Results"
    if short_description:
        subject_line += f" - {short_description}"
    quoted_description = json.dumps(short_description)
    msg_body = f"Your isoforms plot, described as {quoted_description}, is ready. See the attachment."
